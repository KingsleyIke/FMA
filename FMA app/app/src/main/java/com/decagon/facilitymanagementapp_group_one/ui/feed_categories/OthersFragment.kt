...
import com.decagon.facilitymanagementapp_group_one.databinding.FragmentOthersBinding
...
class OthersFragment : Fragment(), ItemClickListener {
    ...
    override fun onCreateView(
        ...
        _binding = FragmentOthersBinding.inflate(layoutInflater)

        adapter.listener = this

        // Load feeds data into the recyclerView
        adapter.loadFeeds(filterFeedCategory(feedsList, "Others"))

        // Attach a recyclerView adapter to the recyclerView
        _binding!!.fragmentOthersFeedsListRv.feedsList.adapter = adapter

        // Adding vertical lines between two views items on the recyclerView
        _binding!!.fragmentOthersFeedsListRv.feedsList.addItemDecoration(
